...
const PREFERENCE_SCHEMA = [
  ...
  { key: 'flashRandom',       type: 'bool',   default: false,   domId: 'flash-random',             section: 'flash', label: 'Random Order' },
  { key: 'flashAuto',         type: 'bool',   default: true,    domId: 'flash-auto',               section: 'flash', label: 'Auto Advance' },
  { key: 'flashFront',        type: 'select', default: 'ja',    domId: 'flash-front',              section: 'flash', label: 'Front', presetBehavior: 'target' },
  { key: 'flashBack1',        type: 'select', default: 'ja_furi',domId: 'flash-back-1',            section: 'flash', label: 'Back 1' },
  { key: 'flashBack2',        type: 'select', default: 'en',    domId: 'flash-back-2',             section: 'flash', label: 'Back 2' },
  { key: 'flashBack3',        type: 'select', default: 'ja_ex', domId: 'flash-back-3',             section: 'flash', label: 'Back 3' },
  { key: 'flashBack4',        type: 'select', default: 'ja_roma',domId: 'flash-back-4',            section: 'flash', label: 'Back 4' },
  ...
  { key: 'quizRandom',        type: 'bool',   default: false,   domId: 'quiz-random',              section: 'quiz', label: 'Random Order' },
  { key: 'quizAuto',          type: 'bool',   default: true,    domId: 'quiz-auto',                section: 'quiz', label: 'Auto Advance' },
  ...
  // ===== TRUE/FALSE =====
  { key: 'tfRandom',          type: 'bool',   default: false,   domId: 'tf-random',                section: 'tf', label: 'Random Order' },
  { key: 'tfAuto',            type: 'bool',   default: true,    domId: 'tf-auto',                  section: 'tf', label: 'Auto Advance' },
  ...
  // ===== MATCH =====
  { key: 'matchHint',         type: 'bool',   default: true,    domId: 'match-hint',               section: 'match', label: 'Flash Hint' },

  // ===== VOICE =====
  { key: 'voiceAuto',         type: 'bool',   default: true,    domId: 'voice-auto',               section: 'voice', label: 'Auto Advance' },
  { key: 'voiceRandom',       type: 'bool',   default: false,   domId: 'voice-random',             section: 'voice', label: 'Random Order' },
  ...
  { key: 'sentencesAuto',     type: 'bool',   default: true,    domId: 'sentences-auto',           section: 'sentences', label: 'Auto Advance' },
  { key: 'sentencesRandom',   type: 'bool',   default: false,   domId: 'sentences-random',         section: 'sentences', label: 'Random Order' },
  ...
  // ===== LLM / AI =====
  { key: 'llmModel',          type: 'select', default: 'gemma4:31b',domId: 'llm-model',             section: 'llm', label: 'Model' },
...
function getLangPrefs(LANG_CONFIG) {
  ...
  for (var i = 0; i < LANG_CONFIG.length; i++) {
    var l = LANG_CONFIG[i];
    if (l.visualOnly) continue;
    var cap = l.key.charAt(0).toUpperCase() + l.key.slice(1);
    prefs.push(
...
function buildDefaultsFromSchema(LANG_CONFIG) {
  var all = getAllPrefs(LANG_CONFIG);
  var obj = {};
...
function readPrefFromDom(entry) {
  ...
  switch (entry.type) {
    case 'bool':
      return el.checked;
    case 'radio': {
      var rad = document.querySelector('input[name="' + entry.domId + '"]:checked');
...
function writePrefToDom(entry, value) {
  ...
  switch (entry.type) {
    case 'bool':
      ...
      break;
    case 'radio': {
      var rad = document.querySelector('input[name="' + entry.domId + '"][value="' + value + '"]');
