...
class TF extends GameMode {
    ...
    playSmartAudio(langKey) {
        ...
        const conf = typeof LANG_MAP !== 'undefined' ? LANG_MAP.get(frontKey) || null : null;
    ...
    update() {
        ...
        if(typeof LANG_CONFIG !== 'undefined') {
            const conf = LANG_MAP.get(frontKey); if(conf) { fSec=c[conf.secondary]||""; fEx=c[conf.exKey]||""; }
            const bConf = LANG_MAP.get(backKey); if(bConf) fExSrc=c[bConf.exKey]||"";
        }

        let exHtml = '';
        if (p.tfShowEx) {
            const exMainConf = LANG_MAP.get(p.tfExMain);
    ...
    check(btn, userChoice) {
        ...
        if(win) {
            ...
            let pAudio = null;
            if (app.store.prefs.tfPlayCorrect) {
                const frontKey = app.store.prefs.tfFront || 'ja';
                pAudio = app.audio.play(this.list[this.i][frontKey], frontKey, 'tf', 0);
            }
