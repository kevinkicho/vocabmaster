...
class Flashcard extends GameMode {
    ...
    triggerAction(action) {
        if (action === 'next') this.nav(1);
            ...
            if(this.dom.card) {
                this.dom.card.classList.toggle('rotate-y-180');
    ...
    playSmartAudio(langKey) {
        ...
        let conf = typeof LANG_MAP !== 'undefined' ? LANG_MAP.get(langKey) : null;
        ...
        this.root.innerHTML = `
            <div class="flex flex-col landscape:flex-row h-full w-full landscape:gap-4 overflow-hidden">
                <div class="flex-1 landscape:flex-1 flex flex-col min-h-0 relative z-10">
                    ...
                    <div class="perspective-1000 w-full flex-1 min-h-0 cursor-pointer group select-none relative pb-2" onclick="if(!event.target.closest('button')) { this.firstElementChild.classList.toggle('rotate-y-180'); app.game.playSmartAudio('${p.flashFront || 'ja'}'); }">
                        <div id="fc-card" class="preserve-3d w-full h-full relative rounded-[2rem] shadow-soft transition-transform" style="transition-duration: ${dur}ms">
                            <div class="absolute inset-0 backface-hidden bg-white dark:bg-neutral-900 rounded-[2rem] border border-slate-100 dark:border-neutral-800 flex flex-col shadow-sm z-10 overflow-hidden gpu-fix">
                            ...
                            <div id="fc-back-container" class="absolute inset-0 backface-hidden bg-slate-800 dark:bg-black text-white rounded-[2rem] rotate-y-180 flex flex-col border border-slate-700 dark:border-neutral-800 shadow-2xl z-10 overflow-hidden gpu-fix">
                            </div>
                        </div>
                    </div>
                </div>
