...
class UIManager {
    ...
    header(curr, total, score, opts = {}) {
        ...
        if (isMatch) {
            ...
            inputHtml = (curr !== null) ? `<div class="flex items-center bg-white dark:bg-neutral-800 rounded-full px-2.5 py-1.5 shadow-sm border border-slate-200 dark:border-neutral-700 mr-auto whitespace-nowrap"><input type="number" value="${curr + 1}" min="1" max="${total}" onchange="app.game.jump(this.value)" onclick="this.select()" class="w-10 text-center font-black text-indigo-600 dark:text-indigo-400 bg-transparent outline-none p-0 text-sm appearance-none rounded" /><span class="text-[10px] font-bold text-slate-400 ml-0.5">/ ${total}</span></div>` : `<div class="flex-1"></div>`;
        }
