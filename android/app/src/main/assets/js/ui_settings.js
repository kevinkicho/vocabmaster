...
Object.assign(UIManager.prototype, {
    ...
    renderPresetsUI() { const container = document.getElementById('preset-container'); if(!container) return; if(!window.app.presets) return; const langs = window.app.presets.languages; const optLabel = (l) => (typeof langOptionLabel === 'function' ? langOptionLabel(l) : l.label); const opts = langs.map(l => `<option value="${l.key}">${optLabel(l)}</option>`).join(''); const p = this.store.prefs; const curSource = p.presetSource || 'en'; const curTarget = p.presetTarget || 'ja'; container.innerHTML = `<div class="grid grid-cols-2 gap-3 mb-3"><div class="flex flex-col"><span class="text-[9px] uppercase font-bold text-slate-400 mb-1">I know...</span><select id="preset-source" class="bg-white dark:bg-neutral-700 border border-slate-200 dark:border-neutral-600 rounded-xl px-3 py-2 text-sm font-bold outline-none shadow-sm text-slate-700 dark:text-neutral-200">${opts}</select></div><div class="flex flex-col"><span class="text-[9px] uppercase font-bold text-slate-400 mb-1">I want to learn...</span><select id="preset-target" class="bg-white dark:bg-neutral-700 border border-slate-200 dark:border-neutral-600 rounded-xl px-3 py-2 text-sm font-bold outline-none shadow-sm text-slate-700 dark:text-neutral-200"><option value="" disabled selected>Select...</option>${opts}</select></div></div><button onclick="app.presets.apply(document.getElementById('preset-source').value, document.getElementById('preset-target').value)" class="w-full bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-2 rounded-xl text-sm shadow-md active:scale-95 active:bg-indigo-700 active:shadow-inner active:brightness-90 transition-all duration-100">Apply Preset</button>`; const src = document.getElementById('preset-source'); const tgt = document.getElementById('preset-target'); if(src) src.value = curSource; if(tgt) tgt.value = curTarget; },
    ...
    createOpts(selId, selectedVal, hideVisuals = false) {
        ...
        html += list.map(l => {
            const lab = (typeof langOptionLabel === 'function' ? langOptionLabel(l) : l.label);
            let str = `<option value="${l.key}" ${l.key===selectedVal?'selected':''}>${lab}</option>`;
            if (l.exKey) {
                str += `<option value="${l.exKey}" ${l.exKey===selectedVal?'selected':''}>↳ ${lab} (Example)</option>`;
            }
            return str;
        }).join('');
        el.innerHTML = html;
