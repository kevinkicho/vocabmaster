...
ANSWER: (letter)`;
    ...
    _validateStoryOutput(fullText, targetWords) {
        if (!fullText) return { valid: false, error: 'Empty output' };

        const extracted = this._extractQuestions(fullText);
        if (!extracted || extracted.length !== 2) {
            return { valid: false, error: `Expected 2 questions but parsed ${extracted ? extracted.length : 0}. Please ensure exactly 2 questions with 4 options each.` };
        }

        let storyPart = this._extractStory(fullText);
...
async _generateStory(prompt, lang) {
        ...
        while (attempt <= maxRetries && !validationResult.valid) {
            ...
            const onToken = (token) => {
                ...
                if (!hitQuestionZone) {
                    if (/\n(?:Q\d|P\d|QUESTION|PERGUNTA|PREGUNTA|1\.)[:\s]?/i.test(fullText)) {
                        hitQuestionZone = true;
                        const cutPoint = fullText.search(/\n(?:Q\d|P\d|QUESTION|PERGUNTA|PREGUNTA|1\.)[:\s]?/i);
                        visibleText = fullText.substring(0, cutPoint).replace(/^STORY:\s*/i, '').replace(/^HISTORIA:\s*/i, '').replace(/^HISTÓRIA:\s*/i, '').replace(/^CUENTO:\s*/i, '').trim();
            ...
            if (fullText.trim().length < 5 && app.llm && !app.llm.useCloud && attempt === 1) {
                L('[Story] 0-length response from first local model, retrying with next preferred candidate');
                ...
                const origResolved = app.llm.resolvedModel;
                app.llm.resolvedModel = next;
                try {
                    fullText = ''; // reset for local fallback
                    await app.llm.streamGenerate({
                        prompt: genPrompt,
                        system: 'You are a language learning assistant. Write simple, clear text suitable for learners.'
                    }, onToken);
                } catch (e) {
                    L('[Story] Local swap also failed:', e);
                } finally {
                    app.llm.resolvedModel = origResolved;
                }
            }
...
_parseAndShow(text, lang) {
    ...
    _extractStory(text) {
        const storyMatch = text.match(/(?:STORY|HISTORIA|HISTÓRIA|CUENTO):\s*([\s\S]*?)(?=\n(?:Q\d|P\d|QUESTION|PERGUNTA|PREGUNTA|1\.)[:\s]?|$)/i);
        return storyMatch ? storyMatch[1].trim() : text.split(/\n(?:Q\d|P\d|QUESTION|PERGUNTA|PREGUNTA|1\.)[:\s]?/i)[0].replace(/^(?:STORY|HISTORIA|HISTÓRIA|CUENTO):\s*/i, '').trim();
    },
